...
export const parseReceiptText = (input) => {
  ...
  const strongLabel = /(transfer(?:red)?\s*amount|deposit\s*amount|transaction\s*amount|total\s*amount|amount\s*paid|paid\s*amount|amount|total|sum|value|debit(?:ed)?|credit(?:ed)?)/i;
  ...
  lines.forEach((line) => {
    ...
    if (/(account|acc\s*no|a\/c|card|reference|ref\s*no|journal|txn|trace|mobile|phone|date|time|balance)/i.test(line) && !hasStrongLabel) {
      return;
    }
  ...
  // ------------------------------------------------------------- reference
  let reference = '';
  const refLabel = /(reference(?:\s*(?:no|number|id))?|ref(?:\s*(?:no|number|id))?|transaction(?:\s*(?:no|id))?|txn(?:\s*(?:no|id))?|journal(?:\s*(?:no|entry))?|trace(?:\s*no)?|receipt(?:\s*no)?|slip(?:\s*no)?|approval(?:\s*code)?|confirmation(?:\s*(?:no|code))?|doc(?:\s*no)?)\s*[:#.\-]?\s*([A-Z0-9][A-Z0-9\-_/]{4,25})/i;
  ...
  const dateMatch =
    ...
    text.match(/\b(\d{1,2}\s?(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s?20\d{2})\b/i);
  ...
  const bankMap = [
    ...
    [['frimi', 'genie', 'ez cash', 'ezcash', 'mcash', 'm-cash', 'helapay'], 'MOBILE_BANKING', 'Mobile Wallet'],
...
const ReceiptScanner = ({
  ...
  const processFile = async (file) => {
    if (!file || (!file.type.startsWith('image/') && !file.name.match(/\.(png|jpg|jpeg|webp|bmp)$/i))) {
    ...
    processReceiptImageToDataUrl(file, async (dataUrl, ocrDataUrl) => {
      ...
      try {
        ...
        if (progressTimerRef.current) clearInterval(progressTimerRef.current);
        setProgress(100);
        setStatusText(ocrFailed
          ? (lang === 'si' ? 'තොරතුරු හඳුනාගත නොහැකි විය' : 'No details detected')
          : (lang === 'si' ? 'OCR සාර්ථකයි!' : 'OCR Complete!'));
              ...
              <div style={{ display: 'grid', gap: '6px' }}>
                {extractedInfo?.amount && (
                  <div className="ocr-data-pill">
                    <span className="ocr-data-label" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                {extractedInfo?.reference && (
                  <div className="ocr-data-pill">
                    ...
                    <span className="ocr-data-val-ref">
                      {extractedInfo.reference}
                    </span>
                  </div>
                )}
                ...
                {extractedInfo?.paymentMethod && (
                  <div className="ocr-data-pill">
                    ...
                    <span style={{ color: 'var(--green)', fontWeight: '700' }}>
                      {extractedInfo.paymentMethod === 'BANK_TRANSFER' ? 'Bank Transfer' : 'Mobile Banking'}
                    </span>
                  </div>
                )}

                {!extractedInfo?.amount && !extractedInfo?.reference && !isScanning && (
                  <div style={{ color: 'var(--muted)', fontSize: '12px', fontStyle: 'italic', padding: '4px 0' }}>
                    {lang === 'si' ? 'ඡායාරූපය සාර්ථකව සම්බන්ධ විය. තොරතුරු පහත Form එකෙන්ද වෙනස් කළ හැක.' : 'Receipt attached. You can adjust details in the form below.'}
                  </div>
                )}
              </div>
            </div>
