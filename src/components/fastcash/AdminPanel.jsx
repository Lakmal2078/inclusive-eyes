...
export default function AdminPanel({ user, setUser, api, notify, lang = 'en', t, move }) {
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      if (res.user?.role !== 'ADMIN') {
        throw new Error(adminT.unauthorizedMsg || 'This account does not have Administrator privileges.');
      }
      setUser(res.user);
  ...
  const handleDeleteAccount = async (idToDelete) => {
    ...
    const confirmed = window.confirm(adminT.deleteConfirm || 'Are you sure you want to delete this bank account?');
    if (!confirmed) return;
  ...
  const handleToggleActive = async (acc) => {
    const updatedList = bankAccounts.map(a => a.id === acc.id ? { ...a, active: !a.active } : a);
  ...
  const filteredTransactions = allTransactions.filter(tx => {
    ...
    if (txSearch.trim()) {
      ...
      const matchRef = (tx.refNumber || tx.journalNumber || tx.reference || '').toLowerCase().includes(q);
  ...
  if (!user || user.role !== 'ADMIN') {
    return (
      <section className="panel" style={{ maxWidth: '520px', margin: '40px auto', padding: '32px' }}>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <h1 style={{ fontSize: '26px', margin: '0 0 8px' }}>{adminT.loginTitle || 'Admin Sign In'}</h1>
          <p style={{ color: 'var(--muted)', fontSize: '14px', margin: 0 }}>
            {adminT.loginSubtitle || 'Enter administrator credentials to manage bank accounts, update settings, and process player requests.'}
          </p>
        </div>
        ...
        <form onSubmit={handleAdminLogin} style={{ display: 'grid', gap: '16px' }}>
          <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
            ...
            <input
              ...
              style={{ padding: '12px 14px', borderRadius: '10px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
            />
          </label>
          ...
          <button
            ...
            className="green"
            disabled={loading}
            style={{ padding: '14px', borderRadius: '10px', fontSize: '15px', fontWeight: '800', marginTop: '6px' }}
          >
        ...
        <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid var(--line)', textAlign: 'center' }}>
          <button
            type="button"
            onClick={() => move('Home')}
            style={{ background: 'transparent', border: 'none', color: 'var(--muted)', fontSize: '13px', cursor: 'pointer' }}
          >
    ...
    <div className="admin-shell">
      ...
      <div className="admin-tabs">
        {[
          ...
          { id: 'transactions', label: `${adminT.tabTransactions || '📥 Transactions'} (${allTransactions.length})` },
          ...
          <button
            ...
            style={{
              ...
              fontWeight: '700',
              whiteSpace: 'nowrap',
              border: activeTab === tab.id ? '1px solid var(--green)' : '1px solid var(--line)',
              background: activeTab === tab.id ? 'var(--green)' : 'var(--panel)',
              color: activeTab === tab.id ? '#102000' : 'var(--ink)',
              cursor: 'pointer',
        ...
        <section style={{ display: 'grid', gap: '24px' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
              <div>
                ...
                <p style={{ color: 'var(--muted)', fontSize: '13px', margin: 0 }}>
                  {adminT.bankAccountsDesc || 'These accounts are displayed live to all users on the Deposit screen. Any additions, updates, or removals take effect immediately.'}
                </p>
              </div>
            ...
            {(isAddingNew || editingAccount) && (
              <form onSubmit={handleSaveAccountForm} style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
                  <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                    ...
                    <input
                      ...
                      style={{ padding: '10px 12px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
                    />
                  </label>
                  ...
                  <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                    ...
                    <input
                      ...
                      onChange={e => setAccountForm(prev => ({ ...prev, number: e.target.value }))}
                      style={{ padding: '10px 12px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
                    />
                  </label>
                  ...
                  <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                    ...
                    <select
                      ...
                      onChange={e => setAccountForm(prev => ({ ...prev, type: e.target.value, icon: e.target.value === 'IPAY' ? '📱' : '🏦' }))}
                      style={{ padding: '10px 12px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
                    >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  />
                  <label htmlFor="accountActiveCheckbox" style={{ fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                    {adminT.accountActive || 'Active (Visible to users on deposit page)'}
                  </label>
                </div>

                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                  <button type="button" onClick={cancelAccountEdit} style={{ padding: '10px 16px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid var(--line)', color: 'var(--ink)', cursor: 'pointer' }}>
                    {adminT.cancelBtn || 'Cancel'}
                  </button>
                  <button type="submit" className="green" disabled={loading} style={{ padding: '10px 20px', borderRadius: '8px', fontWeight: '800' }}>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              {bankAccounts.map((acc, index) => (
                <div
                  key={acc.id || index}
                  style={{
                    background: acc.active === false ? 'rgba(255,255,255,0.01)' : 'rgba(255,255,255,0.04)',
                    border: '1px solid var(--line)',
                    borderRadius: '12px',
                    padding: '16px',
                    display: 'flex',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <strong style={{ fontSize: '16px', color: 'var(--ink)' }}>{acc.name}</strong>
                        {acc.active === false && (
                          <span style={{ fontSize: '10px', background: 'rgba(255,77,77,0.2)', color: '#ff6b6b', padding: '2px 6px', borderRadius: '4px', fontWeight: '700' }}>
                            HIDDEN
                          </span>
                        )}
                        <span style={{ fontSize: '10px', background: 'rgba(34,199,255,0.15)', color: 'var(--blue)', padding: '2px 6px', borderRadius: '4px', fontWeight: '700' }}>
                          {acc.type || 'BANK'}
                        </span>
                      </div>
                      <div style={{ fontSize: '14px', fontFamily: 'monospace', color: 'var(--green)', marginTop: '4px', letterSpacing: '1px', fontWeight: '700' }}>
                        {acc.number}
                      </div>
                    </div>
                  </div>
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '18px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
              <input
                type="text"
                placeholder={adminT.txSearchPlaceholder || 'Search Player ID, Ref, Name...'}
                value={txSearch}
              ...
              <select
                ...
                style={{ padding: '8px 12px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)', fontSize: '13px' }}
              >
          ...
          {filteredTransactions.length === 0 ? (
            ...
            <div style={{ display: 'grid', gap: '14px' }}>
              {filteredTransactions.map(tx => {
                const statusColor = tx.status === 'COMPLETED' ? 'var(--green)' : tx.status === 'REJECTED' ? '#ff6b6b' : tx.status === 'PROCESSING' ? 'var(--blue)' : '#ffc857';
                return (
                  <div
                    key={tx.id}
                    style={{
                  ...
                  >
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                          <span style={{
                            ...
                            padding: '2px 8px',
                            borderRadius: '6px',
                            background: tx.type === 'DEPOSIT' ? 'rgba(182,255,53,0.15)' : 'rgba(34,199,255,0.15)',
                        ...
                        <div style={{ fontSize: '14px', color: 'var(--ink)' }}>
                          ...
                          {tx.refNumber || tx.journalNumber ? ` • Ref: ${tx.refNumber || tx.journalNumber}` : ''}
                          {tx.securityCode ? ` • Sec Code: ${tx.securityCode}` : ''}
                        </div>
                      </div>

                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '18px', fontWeight: '800', color: tx.type === 'DEPOSIT' ? 'var(--green)' : 'var(--blue)' }}>
                    ...
                    {/* Action buttons */}
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center', borderTop: '1px solid var(--line)', paddingTop: '10px' }}>
                      {tx.receiptImage && (
                        <button
                          type="button"
      ...
      {activeTab === 'settings' && (
        <section style={{
          ...
          <p style={{ color: 'var(--muted)', fontSize: '13px', margin: '0 0 20px' }}>
            Update global parameters including the active agent WhatsApp phone number, referral bonus promo code, and allowed transaction limits.
          </p>
          ...
          <form onSubmit={handleSaveSettings} style={{ display: 'grid', gap: '16px' }}>
            <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
              ...
              <input
                ...
                onChange={e => setSystemSettings(prev => ({ ...prev, whatsappNumber: e.target.value }))}
                style={{ padding: '10px 14px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
              />
            </label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                ...
                <input
                  ...
                  onChange={e => setSystemSettings(prev => ({ ...prev, minTransaction: Number(e.target.value) }))}
                  style={{ padding: '10px 14px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
                />
              </label>
              ...
              <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                ...
                <input
                  ...
                  onChange={e => setSystemSettings(prev => ({ ...prev, maxTransaction: Number(e.target.value) }))}
                  style={{ padding: '10px 14px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}
                />
              </label>
            </div>
            ...
            <label style={{ display: 'grid', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
              ...
              <input
                ...
                onChange={e => setSystemSettings(prev => ({ ...prev, promoCode: e.target.value }))}
                style={{ padding: '10px 14px', borderRadius: '8px', background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)', fontWeight: '700', letterSpacing: '1px' }}
              />
            </label>
            ...
            <button
              ...
              className="green"
              disabled={loading}
              style={{ padding: '12px 20px', borderRadius: '10px', fontWeight: '800', fontSize: '14px', marginTop: '10px' }}
            >
      ...
      {activeTab === 'overview' && (
        <section style={{ display: 'grid', gap: '20px' }}>
          <div style={{
            ...
            <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '6px' }}>{adminT.statsUsers || 'Total Registered Users'}</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--ink)' }}>{stats?.users || usersList.length || 0}</div>
            </div>

            <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '6px' }}>{adminT.statsTransactions || 'Total Requests'}</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--blue)' }}>{stats?.transactions || allTransactions.length || 0}</div>
            </div>

            <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '6px' }}>{adminT.statsPending || 'Pending Reviews'}</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: '#ffc857' }}>{stats?.pending || allTransactions.filter(t => t.status === 'PENDING').length || 0}</div>
            </div>

            <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '6px' }}>{adminT.statsCompleted || 'Completed'}</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--green)' }}>{stats?.completed || allTransactions.filter(t => t.status === 'COMPLETED').length || 0}</div>
            </div>
          </div>
        </section>
      )}
      ...
      {selectedSlipImage && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ margin: 0, fontSize: '16px', color: 'var(--ink)' }}>Deposit Receipt Slip</h3>
              <button
                type="button"
                onClick={() => setSelectedSlipImage(null)}
                style={{ background: 'none', border: 'none', color: 'var(--ink)', fontSize: '22px', cursor: 'pointer' }}
              >
                ×
              </button>
            </div>
