...
export const primaryNav = [['🏠','Home'],['⚡','Deposit'],['💳','Withdraw'],['🎲','1xBet'],['📋','Transactions']];
export const moreNav = [['⚽','Sports'],['🎰','Casino'],['🔴','Live Bet'],['🎁','Promotions'],['🛡️','PrivacyPolicy'],['⚙️','Admin'],['💬','Support']];
export const nav = [...primaryNav, ...moreNav];

export const allKnownPages = [
  'Home', 'Deposit', 'Withdraw', '1xBet', 'Transactions',
  'Sports', 'Casino', 'Live Bet', 'Promotions', 'PrivacyPolicy',
  'Admin', 'Support', 'Login', 'Register'
];
...
export const getInitialPage = () => {
  ...
  if (hash) {
    ...
    const matched = allKnownPages.find(
      p => p.toLowerCase() === decoded.toLowerCase() || p.toLowerCase().replace(/\s+/g, '') === decoded.toLowerCase().replace(/\s+/g, '')
    );
...
export const Field = ({ label, name, type, required = true, form, setForm, placeholder, min, max }) => {
    ...
    <>
      <header>
        ...
        <nav>
          ...
          <div className="nav-dropdown-wrap">
            <button 
              ...
              onClick={(e) => { e.stopPropagation(); setMoreOpen(prev => !prev); }}
              aria-expanded={moreOpen}
            >
              {isMoreActive ? (t.nav[page] || page) : (t?.nav?.More || 'More')} <span style={{ fontSize: '10px', opacity: 0.8 }}>{moreOpen ? '▲' : '▼'}</span>
            </button>
            {moreOpen && (
              <div className="nav-dropdown-menu">
                {moreNav.map(([icon, p]) => (
                  <button 
                    ...
                    onClick={() => { move(p); setMoreOpen(false); }}
                    key={p}
                  >
        ...
        <div className="head-actions">
          ...
          <div className="lang-switcher-wrap" style={{ display: 'flex', gap: '2px', background: 'rgba(255,255,255,0.06)', padding: '2px', borderRadius: '10px', border: '1px solid var(--line)' }}>
            <button 
              type="button"
              style={{ padding: '6px 9px', borderRadius: '8px', fontSize: '11px', fontWeight: '700', background: lang === 'en' ? 'var(--blue)' : 'transparent', color: lang === 'en' ? '#001720' : 'var(--muted)', cursor: 'pointer' }}
              onClick={() => setLang('en')}
              title="English"
            >
              EN
            ...
            <button 
              type="button"
              style={{ padding: '6px 9px', borderRadius: '8px', fontSize: '11px', fontWeight: '700', background: lang === 'si' ? 'var(--blue)' : 'transparent', color: lang === 'si' ? '#001720' : 'var(--muted)', cursor: 'pointer' }}
              onClick={() => setLang('si')}
              title="සිංහල"
            >
              SI
            ...
            <button 
              type="button"
              style={{ padding: '6px 9px', borderRadius: '8px', fontSize: '11px', fontWeight: '700', background: lang === 'ta' ? 'var(--blue)' : 'transparent', color: lang === 'ta' ? '#001720' : 'var(--muted)', cursor: 'pointer' }}
              onClick={() => setLang('ta')}
              title="தமிழ்"
            >
              TA
          ...
          {user ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div className="head-user-badge" title={`${user.fullName} (${user.playerId || t.header.notSaved})`}>
            ...
            <button 
              type="button"
              onClick={() => move('Login')}
              style={{ padding: '7px 12px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid var(--line)', color: 'var(--ink)', fontWeight: '700', fontSize: '12px' }}
            >
              {t?.nav?.Login || 'Login'}
            </button>
          )}
      ...
      >
        ...
        <div className="drawer-bottom">
          ...
          <button
            type="button"
            className="theme-toggle-btn drawer-action"
            onClick={toggleTheme}
          >
          ...
          {user ? (
            <div className="drawer-user-card">
              <div className="drawer-user-card__top">
                <div>
                  ...
                  <small className="drawer-user-card__id">{t.header.playerId}: {user.playerId || t.header.notSaved}</small>
                </div>
                {user.role === 'ADMIN' && (
                  <button
                    type="button"
                    className="admin-pill"
                    onClick={() => { move('Admin'); setDrawer(false); }}
                  >
                    ADMIN
                  </button>
                )}
              ...
              <button
                type="button"
                className="logout-btn drawer-action"
                onClick={logout}
              >
                <span>🚪</span> {t?.header?.logout || 'Log Out'}
              </button>
            ...
            <button
              ...
              onClick={() => { move('Login'); setDrawer(false); }}
            >
          ...
          <button
            ...
            onClick={() => { move('Support'); setDrawer(false); }}
          >
  ...
  return (
    <div className={`bank-card ${isIpay ? 'bank-card--ipay' : 'bank-card--bank'}`}>
      <div className="bank-card__head">
        <span className="bank-badge" aria-hidden="true">{initials}</span>
...
export const DepositInstructionsCard = ({ notify, lang, t }) => (
  <div className="agent-details-card instructions-card instructions-card--green">
    ...
    <div className="instructions-body">
      ...
      <div className="instructions-note instructions-note--green">
...
export const WithdrawalInstructionsCard = ({ notify, lang, t }) => (
  <div className="agent-details-card instructions-card instructions-card--blue">
    ...
    <div className="instructions-body">
      ...
      <div className="instructions-note instructions-note--blue">
...
export const getWhatsAppUrl = (tx, lang = 'en', user = null) => {
  ...
  if (lang === 'si') {
    if (isDeposit) {
      ...
      message = `💳 *නව මුදල් ලබාගැනීමේ ඉල්ලීම - FAST CASH*\n` +
        `----------------------------------\n` +
        `🔖 *Ref ID:* #${refId}\n` +
        `👤 *Player ID:* ${resolvedPlayerId}\n` +
        (resolvedUserId && resolvedUserId !== 'GUEST' ? `🆔 *User ID:* ${resolvedUserId}\n` : '') +
    ...
    if (isDeposit) {
      message = `⚡ *புதிய வைப்பு கோரிக்கை - FAST CASH*\n` +
        `----------------------------------\n` +
        `🔖 *Ref ID:* #${refId}\n` +
        `👤 *Player ID:* ${resolvedPlayerId}\n` +
        (resolvedUserId && resolvedUserId !== 'GUEST' ? `🆔 *User ID:* ${resolvedUserId}\n` : '') +
      ...
      message = `💳 *புதிய பணப் பரிமாற்ற கோரிக்கை - FAST CASH*\n` +
        `----------------------------------\n` +
        `🔖 *Ref ID:* #${refId}\n` +
        `👤 *Player ID:* ${resolvedPlayerId}\n` +
        (resolvedUserId && resolvedUserId !== 'GUEST' ? `🆔 *User ID:* ${resolvedUserId}\n` : '') +
...
export const Support = ({ chat, chatText, setChatText, sendChat, config, notify, lang, t }) => {
  ...
  return (
    <section className="support">
      ...
      <div className="chat" ref={chatRef} role="log" aria-live="polite" aria-label={t.supportPage.assistantTitle}>
  ...
  <section className="wizard" style={{ maxWidth: '540px', margin: '30px auto' }}>
    ...
    {user ? (
      <div className="agent-details-card" style={{ marginTop: '20px' }}>
        <p style={{ margin: '0 0 12px', color: 'var(--muted)', fontSize: '14px' }}>{t.login.alreadyLoggedIn || 'You are currently signed in as:'}</p>
        <div className="agent-detail-row" style={{ marginBottom: '18px', padding: '14px' }}>
          <div>
            <strong className="detail-value" style={{ fontSize: '18px', display: 'block' }}>👤 {user.fullName}</strong>
            <div style={{ color: 'var(--muted)', fontSize: '13px', marginTop: '4px' }}>
              {user.email} {user.playerId ? `• ID: ${user.playerId}` : ''}
            </div>
          </div>
          {user.role === 'ADMIN' && (
            <span style={{ fontSize: '11px', background: 'rgba(182,255,53,0.18)', border: '1px solid var(--green)', color: 'var(--green)', padding: '3px 8px', borderRadius: '6px', fontWeight: '800' }}>ADMIN</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <button type="button" className="green" onClick={() => move('Home')} style={{ padding: '10px 18px', borderRadius: '10px', fontWeight: '800' }}>
            {t.nav.Home}
          </button>
          <button type="button" className="logout-btn" onClick={logout} style={{ padding: '10px 18px', borderRadius: '10px', fontSize: '13px' }}>
...
export const Register = ({ form, setForm, notify, setUser, move, t }) => (
  <section className="wizard" style={{ maxWidth: '540px', margin: '30px auto' }}>
    ...
    <form onSubmit={async e => {
      ...
      <Field label={t.register.passwordLabel} name="password" type="password" placeholder="Minimum 6 characters" form={form} setForm={setForm} />
...
export const PwaInstallBanner = ({ t }) => {
  ...
  useEffect(() => {
    const isDismissed = typeof window !== 'undefined' && window.localStorage ? window.localStorage.getItem('fast_cash_pwa_dismissed') : null;
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        window.localStorage.setItem('fast_cash_pwa_dismissed', '1');
      }
    }
  };
