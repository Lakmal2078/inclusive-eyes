...
type FastCashState = {
  ...
  toast: string;
  config: { minTransaction: number; maxTransaction: number; whatsappNumber?: string; promoCode?: string };
  transactions: any[];
...
export function FastCashProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<FastCashState>(
    ...
    [t, lang, theme, user, toast, config, transactions, form, chat, chatText, page, drawer, waModalTx],
  );
