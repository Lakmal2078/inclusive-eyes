...
function pickAmount(lines: Line[]): ExtractedField {
  ...
  for (const line of lines) {
    ...
    const matches = line.text.matchAll(/(?:lkr|rs\.?)?\s*([0-9]{1,3}(?:,[0-9]{3})+(?:\.[0-9]{2})?|[0-9]+\.[0-9]{2})/gi);
    for (const m of matches) {
      ...
      if (!best || score > best.score) {
        best = {
          value: numeric.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
          score,
